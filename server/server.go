...
import (
	...
	"github.com/kungze/quic-tun/server/config"
...
func (s preparedServer) Run() error {
	...
	log.InfoS("Server endpoint start up successful", "listen address", listener.Addr())
	for {
		// Wait client endpoint connection request.
		session, err := listener.Accept(context.Background())
		if err != nil {
			log.ErrorS(err, "Encounter error when accept a connection.")
		} else {
			parent_ctx := context.WithValue(context.TODO(), constants.CtxRemoteEndpointAddr, session.RemoteAddr().String())
			logger := log.NewKlogr().WithValues(constants.ClientEndpointAddr, session.RemoteAddr().String())
			...
			go func() {
				for {
					...
					ctx := log.NewContext(parent_ctx, logger)
					...
					ctx = log.NewContext(ctx, logger.WithValues(constants.ServerAppAddr, (*tun.Conn).RemoteAddr().String()))
