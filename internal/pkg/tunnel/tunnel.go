...
import (
	...
	"github.com/kungze/quic-tun/internal/pkg/classifier"
...
func (t *tunnel) stream2Conn(logger log.Logger, wg *sync.WaitGroup) {
	...
	if err != nil {
		logger.Errorf("Can not forward packet from QUIC stream to TCP/UNIX socket: %s", err.Error())
	}
}
...
func (t *tunnel) conn2Stream(logger log.Logger, wg *sync.WaitGroup) {
	...
	if err != nil {
		logger.Errorf("Can not forward packet from TCP/UNIX socket to QUIC stream: %s", err.Error())
	}
}
